...
public class NewIP {
	...
	protected void createContents(int ngonngu) {
		...
		shell.setImage(SWTResourceManager.getImage(NewIP.class, "/cctv/Icon/IP.ico"));
		shell.setSize(1086, 560);
		shell.setText("New IP");
		...
		btnLuu.setImage(SWTResourceManager.getImage(NewIP.class, "/cctv/Icon/bieutuong/save.png"));
					...
					if (validate(textIP.getText())) {
						try {
							...
							try {
								...
								String insertIP = "INSERT INTO DanhSachIP(Ten,MaLocation,IP,MaLoaiMay,GhiChu,NgayCapNhat) VALUES (N'"
										+ textTen.getText() + "','" + makhuvuc + "','" + textIP.getText() + "','"
										+ maloaimay + "',N'" + textghichu.getText() + "',GETDATE())";
								...
								if (result > 0) {
									try {
										...
										if (ngonngu == 0) {
											...
											thongbao.setMessage("Save success!");
										}
										thongbao.open();
									} catch (Exception ex) {
